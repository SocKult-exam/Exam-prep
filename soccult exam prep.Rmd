...
qual <- qual %>% 
  subset(EndDate != "2021-04-22 10:54:05") #remove Helle
...
clean <- clean %>% 
  ...
  mutate(anagram_accuracy = ifelse(anagram_answer == "bogrod", 1, anagram_accuracy)) %>% 
  mutate(anagram_accuracy = ifelse(anagram_answer == "bog rod", 1, anagram_accuracy)) %>% 
  mutate(anagram_accuracy = ifelse(anagram_answer == "apelsin", 1, anagram_accuracy)) %>% 
...
#right$Education <- str_replace_all(right$Education, "[:punct:]", "")
...
clean <- clean %>% 
  ...
  mutate(AVG_IQ_Ability = (IQ_abilities_1+IQ_abilities_2+IQ_abilities_3)/3)
...
#write_csv(clean, "final_data_sockult")
```
